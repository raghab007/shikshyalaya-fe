...
const UserProfile = () => {
  ...
  const fetchCourses = async () => {
    try {
      ...
      setCourses(response.data); // Set courses from the backend
    } catch (error) {
  ...
  const NavItem = ({ icon, label, link, isActive, onClick }) => (
    <li 
                ...
                {courses.map((course) => (
                  <div key={course.courseID} className="border border-gray-200 rounded-lg overflow-hidden hover:shadow-md transition duration-300">
                    <div className="h-32 bg-gray-100 flex items-center justify-center">
                      <img
                        src={course.imageUrl || "https://via.placeholder.com/150"} // Use course image from backend or a placeholder
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            ...
            <div className="hidden md:flex items-center space-x-4">
              <span className="text-gray-600">{state.firstName} {state.lastName}</span>
              <button
                onClick={logout}
                className="px-3 py-1 text-sm bg-red-500 hover:bg-red-600 text-white rounded transition duration-300"
              >
                Logout
              </button>
            </div>
